...
test("fresh existing-identity path leads with private-key recovery", async ({
  ...
  await expect(
    page.getByText(`Paste your private key to sign in to ${PRODUCT_NAME}.`),
  ).toBeVisible();
  ...
  await expect(
    phoneDialog.getByRole("heading", {
      name: `Use your ${PRODUCT_NAME} identity`,
    }),
  ).toBeVisible();
...
test("first-community owner can create and connect a hosted community", async ({
  ...
  await expect(
    page.getByRole("heading", { name: `Finish connecting ${PRODUCT_NAME}` }),
  ).toBeVisible();
...
test("first-community owner can replace a mismatched account identity", async ({
  ...
  await expect(
    page.getByRole("heading", {
      name: `This account uses a different ${PRODUCT_NAME} identity`,
    }),
  ).toBeVisible();
  await page
...
test("first-run onboarding posts the live Fizz kickoff", async ({ page }) => {
  ...
  await expect(page.getByTestId("message-timeline")).not.toContainText(
    `Hi Morty QA, I'm Fizz. Welcome to ${PRODUCT_NAME}.`,
  );
  await publishWelcomeTeamPresence(page);
